(function($,window,document,undefined){"use strict";var pluginName="imageSelector";var defaults={zoneTemplate:'<div class="zone"></div>',removeTemplate:'<span class="remove">x</span>',draggingClass:'dragging',busyClass:'busy',selected:null,removed:null};function ImageSelector(element,options){this.element=element;this.settings=$.extend({},defaults,options);this._defaults=defaults;this._name=pluginName;this.init()}
$.extend(ImageSelector.prototype,{$el:null,$container:null,$zone:null,$remove:null,$dummyFileInput:null,busy:!1,init:function(){this.$el=$(this.element).hide();this.$container=$('<div class="image-selector"></div>');this.$zone=$(this.settings.zoneTemplate).prop('contenteditable',!0);this.$container.append(this.$zone);this.$el.after(this.$container);this.preventKeyInput();this.addPasteEvent();this.addDropEvent();this.addDoubleClickEvent()},addPasteEvent:function(){var _this=this;this.$zone.bind('paste',function(e){e.preventDefault();var clipboardData=e.clipboardData||e.originalEvent.clipboardData||null;if(clipboardData){var items=clipboardData.items||null;if(items){for(var i=0,length=items.length;i<length;i++){if(items[i].kind==='string'){items[i].getAsString(function(url){_this.loadImageFromUrl(url)})}else if(_this.isMimeTypeImage(items[i].type)){_this.loadImage(items[i].getAsFile())}}}}
...
this.setBusy(!0);var _this=this;setTimeout(function(){var reader=new FileReader();reader.onload=function(e){return _this.imageSelected(e.target.result)};reader.readAsDataURL(file)},1)},loadImageFromUrl:function(url){if(!this.isUrl(url)){return!1}
if(this.isBusy()){return!1}
this.setBusy(!0);var _this=this;var xhr=new XMLHttpRequest();xhr.onload=function(){var reader=new FileReader();reader.onloadend=function(){_this.imageSelected(reader.result)};reader.readAsDataURL(xhr.response);_this.setBusy(!1)};xhr.open('GET',url);xhr.responseType='blob';xhr.send()},imageSelected:function(imageData){this.setBusy(!1);this.$el.val(imageData);this.updatePreview(imageData);var _this=this;if(!this.$remove){this.$remove=$(this.settings.removeTemplate).click(function(e){e.preventDefault();e.stopPropagation();_this.updatePreview(null);_this.$el.val('');$(this).remove();_this.$remove=null;if(_this.settings.removed&&typeof _this.settings.removed==='function'){_this.settings.removed()}
return!1});this.$container.append(this.$remove)}
if(typeof this.settings.selected==='function'){this.settings.selected(imageData)}},updatePreview:function(imageData){this.$zone.css({'background-repeat':'no-repeat','background-position':'center center','background-size':'cover','background-image':imageData?'url(\''+imageData+'\')':'none'})},preventKeyInput:function(){var events=['keyup','keydown','keypress'];for(var i in events){var event=events[i];this.$zone[event](function(e){if((!e.ctrlKey&&!e.metaKey)||e.key!=='v'){e.preventDefault();return!1}})}}});$.fn[pluginName]=function(options){return this.each(function(){if(!$.data(this,"plugin_"+pluginName)){$.data(this,"plugin_"+pluginName,new ImageSelector(this,options))}})}})(jQuery,window,document)